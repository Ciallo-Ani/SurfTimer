...
public void SQL_GetMapSettings_Callback(Database db, DBResultSet results, const char[] error, any data)
{
	if(results == null)
	{
		LogError("Timer (tier::get map settings) error! Reason: %s", error);
	...
	{
		DB_InsertMapSettings(gCV_DefaultTier.IntValue, gCV_DefaultMaxvelocity.FloatValue);
	}

	DB_FillTierCache();
}

void DB_InsertMapSettings(int tier, float maxvel)
{
	gI_Tier = tier;
	gB_Maplimitspeed = true;
	gCV_Maxvelocity.FloatValue = maxvel;

	char sQuery[256];
	FormatEx(sQuery, sizeof(sQuery), mysql_InsertMapSettings, gS_Map, tier, maxvel);
	gH_SQL.Query(SQL_InsertMapSettings_Callback, sQuery, 0, DBPrio_High);
}

public void SQL_InsertMapSettings_Callback(Database db, DBResultSet results, const char[] error, any data)
{
	if(results == null)
	{
		LogError("Timer (tier::Insert MapSettings) error! Reason: %s", error);
...
public void SQL_FillTierCache_Callback(Database db, DBResultSet results, const char[] error, any data)
{
	if(results == null)
	{
		LogError("Timer (tier::fill tier cache) error! Reason: %s", error);
...
public void SQL_DeleteMapSettings_Callback(Database db, DBResultSet results, const char[] error, any data)
{
	if(results == null)
	{
		LogError("Timer (tier::deletemap) SQL query failed. Reason: %s", error);
...
public void SQL_ModifyDefMaxvel_Callback(Database db, DBResultSet results, const char[] error, any data)
{
	if(results == null)
	{
		LogError("Timer (tier::modify column maxvelocity) error! Reason: %s", error);
...
{
	char sQuery[512];
	FormatEx(sQuery, sizeof(sQuery), mysql_SetMapTier, tier, gS_Map);
...
public void SQL_SetMapTier_Callback(Database db, DBResultSet results, const char[] error, any data)
{
	if(results == null)
	{
		LogError("Timer (tier::set map tier) error! Reason: %s", error);
...
public void SQL_SetMapSettings_Callback(Database db, DBResultSet results, const char[] error, any data)
{
	if(results == null)
	{
		LogError("Timer (tier::set map settings) error! Reason: %s", error);
